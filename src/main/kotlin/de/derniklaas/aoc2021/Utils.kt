...
public fun List<String>.splitAndMapToInt(deliminator: String = " ") =
    map { it.split(deliminator).filter(String::isNotEmpty).mapToInt() }

public fun List<String>.splitAndMapToInt(deliminator: Regex) =
    map { it.split(deliminator).filter(String::isNotEmpty).mapToInt() }
